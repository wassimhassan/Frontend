...
const BookingsPage = () => {
    ...
    const handleSelectSession = (trainerId, day, time) => {
        if (isSessionBooked(trainerId, time)) return;
        
        setSelectedSession({ trainerId, day, time });
        setShowSubscriptionChoice(true); // Show subscription modal instead of direct booking
    };
    ...
    const getSessionCost = () => {
        ...
        return selectedTrainer.hourlyRate || 5; // Default to $50 if not specified
    };
    ...
    return (
        <div className="bp-bookings-page">
                            ...
                            {trainers.map((trainer) => (
                                ...
                                >
                                    ...
                                    <p className="bp-trainer-rate">
                                        <strong>Rate:</strong> ${trainer.hourlyRate || 50}/hour
                                    </p>
            ...
            {bookingStep === 2 && selectedTrainer && (
                ...
                <div className="bp-subscription-modal">
                    <div className="bp-subscription-content">
                        ...
                        <p>Save money with a subscription or pay per session.</p>
                        
                        <div className="bp-subscription-buttons">
                            <button 
                                className="bp-subscribe-button"
                                onClick={() => {
                                    // Redirect to subscription page with booking context
                                    navigate("/subscribe?redirectTo=booking&trainerId=" + selectedTrainer._id);
                                }}
                            >
                        ...
                        <button 
                            ...
                            onClick={() => {
                                setShowSubscriptionChoice(false);
                                setBookingStep(1); // Go back to trainer selection
                            }}
                        >
                            Cancel
                        </button>
                    ...
                    {selectedTrainer.availability.length > 0 ? (
                        <div className="bp-availability-calendar">
                            {selectedTrainer.availability.map((slot, index) => (
                                <div key={index} className="bp-day-column">
                                    <h3 className="bp-day-header">{slot.day}</h3>
                                        ...
                                        {slot.time.length > 0 ? (
                                            slot.time.map((time, i) => (
                                                <button
                                                    ...
                                                    className={`bp-time-slot ${
                                                        isSessionBooked(selectedTrainer._id, time) ? "bp-booked" : ""
                                                    }`}
                                                    disabled={isSessionBooked(selectedTrainer._id, time)}
                                                >
                                                    {time.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                                                    {isSessionBooked(selectedTrainer._id, time) && " (Booked)"}
                                                </button>
                                            ))
                                        ) : (
                        ...
                        <p className="bp-no-availability">This trainer has no available time slots.</p>
                    )}
                </div>
            )}
                ...
                <div className="bp-payment-confirmation">
                    ...
                    {!showStripeForm ? (
                        <div className="bp-payment-methods">
                            ...
                            >
                                {paymentMethod === "inPerson" ? "Book Now (Pay Later)" : "Proceed to Payment"}
                            </button>
                        </div>
                    ) : (
            ...
            {/* Bookings List */}
            {/* Replace the existing bookings list section with this enhanced version */}
                ...
                return (
                    ...
                    >
                        <div className="bp-booking-trainer">
                            <strong>{booking.trainerId.username}</strong>
                        </div>
                        ...
                        <div className={`
                            bp-booking-payment 
                            ${isPaid ? 'bp-payment-paid' : 'bp-payment-unpaid'}
                        `}>
                            {isPaid ? "Paid" : "Payment Due"}
                            {!isPaid && isUpcoming && (
                                <span className="bp-payment-warning">⚠️ Pay before session</span>
                            )}
                        </div>
                    </li>
                );
